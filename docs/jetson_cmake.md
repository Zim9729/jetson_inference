...
- `path` 指向总目录
- 程序会自动查找 `run_date` 对应日期前缀的批次目录，例如 `run_date="20260411"` 就匹配 `20260411*`
- 同一天多个批次目录会按名称从小到大依次处理
- 每个批次目录只扫描 `E1` / `E2` / `E3` / `E4`
- 已处理过的文件会通过 checkpoint 自动跳过
- 如果 `run_date` 没有配置，或者格式不合法，程序会告警并回退到当天
...
- 程序会在总目录下查找 `run_date` 对应日期前缀的所有批次目录
- 例如 `run_date="20260411"`，就会匹配 `20260411*`
